...
public class ItemDropUpSpawner : BaseInteractiveObject
{
	...
	public override void Use(BaseHero Sender)
	{
		...
		{
			var rndMax = spawnerEvents.Sum(x => x.randomWeight.Evaluate(x.smartLogic.GetWeight(Sender)));
			var randomEventWeight = Random.Range(0, rndMax);
			foreach (var item in spawnerEvents)
			{
				randomEventWeight -= item.randomWeight.Evaluate(item.smartLogic.GetWeight(Sender));
	...
	void StartSpawner(RandomDropUpSpawnerEvent spawnerEvent, BaseHero Sender)
	{
		...
		{
			var rndMax = spawnerEvent.randomDropUpSpawnerItems.Sum(x => x.randomWeight.Evaluate(x.smartLogic.GetWeight(Sender)));
